...
# Use @lru_cache(maxsize=1) to cache the configs so the file is only read once per process, improving performance.
@lru_cache(maxsize=1)
def load_configs():
    """
...
def get_configs(name, type):
    ...
    Args:
        name (str): The top-level key in the YAML configuration.
        type (str): The sub-key under the top-level key.
...
def recommend_laptops(**args):
    ...
    Args:
        **args: Keyword arguments representing user requirements.
